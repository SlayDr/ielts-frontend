...
const ListeningHistory = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/listening/history`, {
    ...
    <div className="listening-history">
      <header className="history-header">
        <div className="header-content">
          ...
          <p className="subtitle">Review your past listening sessions</p>
        </div>
      </header>
      ...
      <main className="history-main">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>Complete a listening practice to see your history here.</p>
            <button onClick={() => navigate('/listening')}>Start Listening Practice</button>
          </div>
        ) : (
            ...
            {sessions.map((session, index) => (
              <div key={session._id || index} className="session-card">
                <div className="session-header">
                  <h3 className="section-title">{session.sectionTitle}</h3>
